...
import com.silzila.querybuilder.SelectClauseSnowflake; // Import the appropriate SelectClause for Snowflake
import com.silzila.querybuilder.WhereClause;

import java.util.*;

    public class SyncFilterQuerySnowflake {
        ...
        public static String getSyncFilterOptions(List<ColumnFilter> cf, String fromQuery, String venderName) throws BadRequestException {
            try {
                ...
                // Build the SELECT clause using the selected columns and alias map for Snowflake
                QueryClauseFieldListMap selectQuery = SelectClauseSnowflake.buildSelectClause(query, venderName, aliasMap);
                ...
                // Build WHERE clause using the panel for Snowflake
                String whereClause = WhereClause.buildWhereClause(Collections.singletonList(panel), "snowflake");

                // Append the WHERE clause to the final query
                finalQuery.append(whereClause);

                // Print the generated query for debugging
                System.out.println("Generated Snowflake Query: " + finalQuery.toString());
