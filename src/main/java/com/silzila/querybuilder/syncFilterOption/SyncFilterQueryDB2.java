...
import com.silzila.querybuilder.SelectClauseDB2; // Use the appropriate SelectClause for DB2
import com.silzila.querybuilder.WhereClause;

import java.util.*;

    public class SyncFilterQueryDB2 {
        ...
        public static String getSyncFilterOptions(List<ColumnFilter> cf, String fromQuery, String vendorName) throws BadRequestException {
            try {
                ...
                // Build the SELECT clause using the selected columns and alias map for DB2
                QueryClauseFieldListMap selectQuery = SelectClauseDB2.buildSelectClause(query, vendorName, aliasMap);
                ...
                // Build WHERE clause using the panel for DB2
                String whereClause = WhereClause.buildWhereClause(Collections.singletonList(panel), "db2");

                // Append the WHERE clause to the final query
                finalQuery.append(whereClause);

                // Print the generated query for debugging
                System.out.println("Generated DB2 Query: " + finalQuery.toString());
