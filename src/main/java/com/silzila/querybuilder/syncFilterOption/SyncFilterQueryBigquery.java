...
import com.silzila.querybuilder.SelectClauseBigquery;
import com.silzila.querybuilder.WhereClause;

import java.util.*;

public class SyncFilterQueryBigquery {
    ...
    public static String getSyncFilterOptions(List<ColumnFilter> cf, String fromQuery, String venderName) throws BadRequestException {
        try {
            ...
            // Build the SELECT clause using the selected columns and alias map for BigQuery
            QueryClauseFieldListMap selectQuery = SelectClauseBigquery.buildSelectClause(query, venderName, aliasMap);
            ...
            // Build WHERE clause using the panel for BigQuery
            String whereClause = WhereClause.buildWhereClause(Collections.singletonList(panel), "bigquery");

            // Append the WHERE clause to the final query
            finalQuery.append(whereClause);

            // Print the generated query for debugging
            System.out.println("Generated BigQuery Query: " + finalQuery.toString());
