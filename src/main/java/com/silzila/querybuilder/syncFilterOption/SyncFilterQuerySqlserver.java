...
import com.silzila.querybuilder.SelectClauseSqlserver;
import com.silzila.querybuilder.WhereClause;

import java.util.*;

public class SyncFilterQuerySqlserver {
    ...
    public static String getSyncFilterOptions(List<ColumnFilter> cf, String fromQuery, String venderName) throws BadRequestException {
        try {
            ...
            // Create Query objectsudo cat /var/opt/mssql/log/errorlog
            ...
            QueryClauseFieldListMap selectQuery = SelectClauseSqlserver.buildSelectClause(query, venderName, aliasMap);
            ...
            String whereClause = WhereClause.buildWhereClause(Collections.singletonList(panel), "sqlserver");

            // Append the WHERE clause to the final query
            finalQuery.append(whereClause);

            // Print the generated query for debugging
            System.out.println("Generated SQL Server Query: " + finalQuery.toString());
