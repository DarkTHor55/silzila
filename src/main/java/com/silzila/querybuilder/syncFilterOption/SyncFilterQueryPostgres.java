...
import com.silzila.querybuilder.SelectClausePostgres; // Import PostgreSQL specific class
import com.silzila.querybuilder.WhereClause;

import java.util.*;

public class SyncFilterQueryPostgres {

    public static String getSyncFilterOptions(List<ColumnFilter> cf, String fromQuery, String venderName) throws BadRequestException {
        try {
            ...
            for (ColumnFilter columnFilter : cf) {
                ...
                if (!columnFilter.getCurrentSelection()) {
                    // PostgreSQL case-sensitivity: wrap column names in quotes if necessary
            ...
            // Build the SELECT clause using the selected columns and alias map (PostgreSQL specific)
            QueryClauseFieldListMap selectQuery = SelectClausePostgres.buildSelectClause(query, venderName, aliasMap);
            ...
            String whereClause = WhereClause.buildWhereClause(Collections.singletonList(panel), "postgres"); // Use PostgreSQL as vendor here

            // Append the WHERE clause to the final query
            finalQuery.append(whereClause);

            // Print the generated query for debugging
            System.out.println("Generated PostgreSQL Query: " + finalQuery.toString());
