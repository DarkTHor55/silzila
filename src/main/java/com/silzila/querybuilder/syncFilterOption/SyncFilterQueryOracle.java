...
import com.silzila.querybuilder.SelectClauseOracle;
import com.silzila.querybuilder.WhereClause;

import java.util.*;

public class SyncFilterQueryOracle {
    ...
    public static String getSyncFilterOptions(List<ColumnFilter> cf, String fromQuery, String vendorName) throws BadRequestException {
        try {
            ...
            QueryClauseFieldListMap selectQuery = SelectClauseOracle.buildSelectClause(query, vendorName, aliasMap);
            ...
            String whereClause = WhereClause.buildWhereClause(Collections.singletonList(panel), "oracle");

            // Append the WHERE clause to the final query
            finalQuery.append(whereClause);

            // Print the generated query for debugging
            System.out.println("Generated Oracle Query: " + finalQuery.toString());
